...
async function fetchEMSCEarthquakes(): Promise<Earthquake[]> {
    try {
        ...
        console.log('   📡 Fetching from EMSC:', url.substring(0, 100) + '...');
        ...
        if (!response.ok) {
            console.error(`   ❌ EMSC returned ${response.status}`);
            throw new Error(`EMSC error: ${response.status}`);
        }
        ...
        if (!text || text.length === 0) {
            console.log('   ⚠️  EMSC returned empty response');
            return [];
        }
        ...
        if (!data.features?.length) {
            console.log('   ℹ️  No earthquakes found in response');
            return [];
        }
        ...
        // Don't throw - just return empty array so we continue checking
        return [];
    }
}
            ...
            try {
                const res = await fetch(PUSH_NOTIFICATION_URL, {
                    ...
                    body: JSON.stringify({
                        ...
                        title: title,
                        body: body,
                        type: 'earthquake',  // ← New field
                        severity: quake.magnitude >= 7.0 ? 'critical' : 'high',  // ← New field
                        data: {
                            magnitude: quake.magnitude,
