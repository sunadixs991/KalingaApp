...
export default function ProfileScreen() {
  ...
  // --- IMPROVED Profile Picture Logic with expo-image-picker ---
  ...
  const uploadProfilePicture = async (asset) => {
    try {
      console.log("Uploading asset URI:", asset.uri);
      
      // Get user identifier for filename
      let userIdentifier = await AsyncStorage.getItem("user");
      if (!userIdentifier) {
      ...
      try {
        const parsed = JSON.parse(userIdentifier);
        if (typeof parsed === "object" && parsed !== null) {
          userId = parsed.username || parsed.email || parsed.id || userIdentifier;
        }
      } catch (e) {
      ...
      // APPROACH 1: Try FormData approach first (works better in React Native)
      try {
        ...
        await handleUploadSuccess(fileName, userId);
        return;
        
      } catch (formDataError) {
        console.log("FormData approach failed, trying FileSystem approach:", formDataError);
        
        // APPROACH 2: Fallback to FileSystem + base64 approach
        ...
        console.log("FileSystem upload successful:", data);
        await handleUploadSuccess(fileName, userId);
      }
  ...
  const handleUploadSuccess = async (fileName, userId) => {
    // Get public URL
    ...
    if (!userSnap.empty) {
      ...
      console.log("Firestore updated with new profile URL");
    }
