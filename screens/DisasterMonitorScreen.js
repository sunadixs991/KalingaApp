...
let checkMonitoringStatus, triggerManualDisasterCheck, getRecentDisasterSummary, getDisasterStatistics, testDisasterNotification;

try {
  const module = require('../services/DisasterNotificationService');
  checkMonitoringStatus = module.checkMonitoringStatus;
  triggerManualDisasterCheck = module.triggerManualDisasterCheck;
  getRecentDisasterSummary = module.getRecentDisasterSummary;
  getDisasterStatistics = module.getDisasterStatistics;
  testDisasterNotification = module.testDisasterNotification;
} catch (e) {
  console.error('Failed to load DisasterNotificationService:', e);
}
...
export default function DisasterMonitorScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const status = await checkMonitoringStatus?.() || { active: false };
      const summary = await getRecentDisasterSummary?.() || { earthquakes: { count: 0 }, weatherAlerts: { count: 0 }, totalAlerts: 0 };
      const stats = await getDisasterStatistics?.(30) || { totalEvents: 0, earthquakes: 0, weatherAlerts: 0 };
